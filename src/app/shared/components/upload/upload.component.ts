...
import { SettingsServiceService } from '@app/views/settings/service/settings-service.service'
  ...
  export class UploadComponent implements OnInit {
    ...
    constructor(
        ...
        private configurationService:SettingsServiceService
    ) {
    }
    ...
    changeHandle(event: Event): void {
        const customizedFile = (<HTMLInputElement>event.target).files[0];
        const formData = new FormData();
        formData.append('image', customizedFile)
        ...
        checkedFiles.forEach((file: File) => {
            ...
            // this.uploadInterceptor(customizedFile) === false ? this.removeHandle(next) : this.upload(next)
        })
    }

    public uploadImage(file) {
      this.configurationService.uploadConfigurationsImage(file)
        .subscribe({
          next: (image: any) => {
            this.configurationService.setImageUrl(image)
          }
        })
    }
    upload(file: CommonFile): void {
        ...
        this.configurationService.uploadConfigurationsImage(null).subscribe((event: any) => {
